...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            double_preserve_mask = (
                torch.cat([preserve_mask, preserve_mask], dim=0)
            ...
            double_proj = torch.cat(
                [cond_cache["audio_projected"], uncond_cache["audio_projected"]], dim=0
            )
            double_k_all = torch.cat(
                [cond_cache["audio_k_all"], uncond_cache["audio_k_all"]], dim=0
            )
            double_v_all = torch.cat(
                [cond_cache["audio_v_all"], uncond_cache["audio_v_all"]], dim=0
            )
    ...
    def build_audio_cache(self, audio_embeddings: torch.Tensor) -> dict:
        ...
        return {
            "audio_projected": projected_audio,
            "audio_k_all": k_all,
            "audio_v_all": v_all,
        }
